...
一場賽事會依序找四個地方，找到就停，最多用 4 次 AI 呼叫：
表上那個連結的頁面 → 頁面指出的更深一層專屬頁 → **賽程 PDF**（最多 2 份）→ 系列官網（`seriesLinks`）。
賽程 PDF 是 2026-09-19 加的：KPC、Red Dragon、USOP 的賽程只放在 PDF 裡，網頁上只有連結。
PDF 先抽文字層（乾淨的話送文字，一整頁塞三百列的大表當圖片會看錯行）；文字層壞掉（字型沒帶對照表、數字全變 �）
或整頁是圖的，才整包送給 Gemini 當圖片看。AI 只負責把所有主賽事列出來，挑哪一個由程式決定（有品牌字樣就對品牌，
活動節名就取保證獎金最大的 Main Event）；
檔案上限 10 MB，會挑檔名或連結文字有 schedule 的，跳過 player guide／rules／policy 那些。
log 裡「📄 買入來自賽程 PDF」就是這條路補到的。
...
**Q：怎麼確認改動沒把東西弄壞？**
`cd scraper && node test.mjs`——343 項純邏輯測試，不需要金鑰也不連網。GitHub Actions 每次跑之前也會先跑一遍。
